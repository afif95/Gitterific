...
public class CommitActor extends AbstractActor {
    ...
    public CommitActor(final ActorRef wsOut, WSClient wsc, Http.Session session) {
    	ws =  wsOut;
    	this.wsc =  wsc;
    ...
    public static Props props(final ActorRef wsout, WSClient wsc,Http.Session session) {
        return Props.create(CommitActor.class, wsout, wsc, session);
    }
    ...
    private void fetchResults(JsonNode js){
		...
		try {
			...
			gh.fetchCommitImp(userSearches, searchVal, ws, wsc, self(), session, singleton)
			 .thenApply(r -> {
		            final ObjectNode response = Json.newObject();
		            final Map<String,String> a1 = new HashMap<>();
